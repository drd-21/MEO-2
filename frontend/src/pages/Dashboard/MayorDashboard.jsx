...
export default function MayorDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mayorApps = allApps.filter((app) => {
        ...
        return (
          // Primary: needs Mayor action
          app.status === "Pending Mayor" ||

          // Items forwarded back to MEO but mention mayor (i.e., mayor endorsement returned)
          (app.status === "Pending MEO" && commentsStr.includes("mayor")) ||

          // Already approved/permitted but previously handled by mayor (for records)
          ((app.status === "Approved" || app.status === "Permit Issued") && commentsStr.includes("mayor")) ||

          // Rejected by mayor specifically
          (app.status === "Rejected" && commentsStr.includes("mayor"))
        );
      });
  ...
  const stats = useMemo(() => {
    ...
    // Endorsed back to MEO and mentions mayor in history
    const endorsed = applications.filter(a =>
      a.status === "Pending MEO" &&
      (a.workflowHistory || []).some(h => (h.comments || "").toLowerCase().includes("mayor"))
    ).length;

    // Completed (Approved / Permit Issued) but previously handled by mayor
    const completed = applications.filter(a =>
      (["Approved", "Permit Issued"].includes(a.status)) &&
      (a.workflowHistory || []).some(h => (h.comments || "").toLowerCase().includes("mayor"))
    ).length;
  ...
  // --- Recent activities (mayor-related) ---
  const recentActivities = useMemo(() => {
    ...
    return applications
      ...
      .filter(e => (e.comments || "").toLowerCase().includes("mayor"))
  ...
  const handleUpdateApplication = async (appId, status, payload = {}) => {
    try {
      ...
      // refresh view to reflect role-specific filters (like BFP)
      await fetchData();

      // If modal open update selectedApp state briefly
