  ...
  if (role === 'mayoradmin') {
    switch (status) {
      case 'Pending Mayor':
        return (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">Review endorsements and approve the mayor's permit.</p>
            <button onClick={() => onUpdate(app._id, 'Approved', { comments: "Mayor: Endorsement approved." })} className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg">
              Approve & Forward for Permit
            </button>
            <button onClick={() => onUpdate(app._id, 'Rejected', { comments: "Mayor: Rejected due to issues." })} className="w-full px-4 py-2 bg-red-500 text-white rounded-lg">
              Reject
            </button>
          </div>
        );
      case 'Approved':
        return (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">Mayor approved. Finalize permit issuance with MEO.</p>
            <button onClick={() => onUpdate(app._id, 'Permit Issued', { comments: 'Permit Issued by Mayor' })} className="w-full px-4 py-2 bg-green-600 text-white rounded-lg">
              Finalize Permit Issuance
            </button>
          </div>
        );
      default:
        return <p className="text-sm text-gray-500">No actions available for this status (Mayor).</p>;
    }
  }
